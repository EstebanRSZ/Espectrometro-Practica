...
# ---------------------------------------------------------------------------
# MODELO GEOMETRICO DEL ESPECTROMETRO
# Ajusta estos valores segun tu montaje fisico real.
#
# GRATING_D_UM      : periodo de la red en micrometros (1000 lineas/mm -> 1.0 um)
# DETECTOR_DIST_MM  : distancia L entre la red y el plano de barrido del detector [mm]
# STEPS_PER_MM      : pasos del motor por milimetro de desplazamiento lineal.
#                     Con 28BYJ-48 (4096 pasos/vuelta) y pinon z=20 dientes (modulo 1):
#                       circunferencia = z*pi mm -> pasos/mm = 4096 / (20*pi) ~ 65.2
# STEP_OFFSET       : numero de pasos desde el home (orden 0) hasta la primera muestra.
#                     Si el barrido empieza justo en el orden 0, vale 0.
#                     Aumenta este valor si el espectro aparece corrido hacia el rojo;
#                     disminuyelo si aparece corrido hacia el azul.
#
# IMPORTANTE: si prefieres el mapeo lineal simple (sin geometria), pon
#   USE_GEOMETRIC_MODEL = False  y ajusta WAVELENGTH_START_NM / WAVELENGTH_END_NM.
# ---------------------------------------------------------------------------
USE_GEOMETRIC_MODEL = True

GRATING_D_UM      = 1.0        # um  (red 1000 lineas/mm)
DETECTOR_DIST_MM  = 100.0      # mm  <- MIDE ESTE VALOR EN TU MONTAJE
STEPS_PER_MM      = 65.2       # pasos/mm  <- ajusta segun tu pinon
# STEP_OFFSET: pasos acumulados entre el home (orden 0) y la PRIMERA muestra
# que el ESP32 envia por serial.
# Con L=100mm y pinon z=20: 400nm requiere ~2846 pasos desde el orden 0.
# Si el motor hace homing al orden 0 y luego avanza hasta la posicion de inicio
# del barrido antes de empezar a enviar datos, pon aqui cuantos pasos avanza.
# Ejemplo: si avanzas 2846 pasos antes de la primera lectura -> STEP_OFFSET = 2846
# Si no hay homing y el barrido empieza desde donde esta el motor -> STEP_OFFSET = 0
# AJUSTA ESTE VALOR segun tu firmware hasta que el pico del laser rojo (650nm)
# aparezca en la posicion correcta.
STEP_OFFSET       = 2846       # <- punto de partida; calibra con el laser rojo

# Rango visible para el eje X cuando USE_GEOMETRIC_MODEL=False
# (tambien se usa como limite de display cuando el modelo geometrico esta activo)
...
def wavelength_for_index(index: int) -> float:
    ...
    Si USE_GEOMETRIC_MODEL=True usa el modelo fisico del espectrometro:
        y = (index + STEP_OFFSET) / STEPS_PER_MM   [desplazamiento en mm]
        theta = arctan(y / DETECTOR_DIST_MM)
        lambda = GRATING_D_UM * 1000 * sin(theta)  [nm, orden m=1]

    Si USE_GEOMETRIC_MODEL=False usa interpolacion lineal simple entre
    WAVELENGTH_START_NM y WAVELENGTH_END_NM.
    """
    if USE_GEOMETRIC_MODEL:
        steps = index + STEP_OFFSET
        y_mm = steps / STEPS_PER_MM
        theta_rad = _math.atan2(y_mm, DETECTOR_DIST_MM)
        lam_nm = GRATING_D_UM * 1000.0 * _math.sin(theta_rad)  # nm
        return max(lam_nm, 0.0)
    else:
...
class SpectrumPlot(pg.PlotWidget):
    ...
    def _make_curve(self) -> pg.PlotCurveItem:
        """Crea y configura un PlotCurveItem nuevo con el estilo del canal."""
        pen = pg.mkPen("#00e5ff", width=1.5) if self._gradient else pg.mkPen("#5a6070", width=1.5)
        brush = self._make_gradient_brush() if self._gradient else QBrush(QColor(90, 96, 112, 38))
    ...
    def set_data(self, x_values: list[float], y_values: list[float]) -> None:
        ...
        # Eje X: durante el barrido muestra solo el rango ya recorrido (con
        # un pequeno margen delantero) para que la curva se vea a escala real.
        # Una vez que se cubre todo el rango visible, fija el rango completo.
        x_min = min(x_values)
        x_max = max(x_values)
        span = max(x_max - x_min, 1.0)
        display_max = min(x_max + span * 0.08, WAVELENGTH_END_NM)
        display_min = max(x_min - span * 0.02, 0.0)
        # Si ya se alcanzo el rango completo configurado, usa ese rango fijo
        if x_max >= WAVELENGTH_END_NM * 0.95:
            display_min = WAVELENGTH_START_NM
            display_max = WAVELENGTH_END_NM

        self.getPlotItem().setXRange(display_min, display_max, padding=0.02)
        self.getPlotItem().setYRange(0, y_top, padding=0)
        self._curve.setData(x=x_values, y=y_values)
        self.update()

    def clear(self) -> None:
        pi = self.getPlotItem()
        # Eliminar la curva vieja completamente y crear una nueva.
        # Esto evita el bug de pyqtgraph donde el brush de gradiente queda
        # invalidado al hacer setData([], []) tras un removeItem/re-addItem.
        if self._curve in pi.items:
            pi.removeItem(self._curve)
        self._curve = self._make_curve()
        pi.addItem(self._curve)
        # Asegurar que el label tambien este en el PlotItem
        if self._empty_label not in pi.items:
            pi.addItem(self._empty_label)
        self._empty_label.setVisible(True)
        pi.setXRange(WAVELENGTH_START_NM, WAVELENGTH_END_NM, padding=0.02)
        pi.setYRange(0, 1, padding=0.05)
        self.update()
...
class Opt101App(QMainWindow):
    ...
    def _build_readouts(self) -> QWidget:
        ...
        self._signal_readout = ReadoutCard("senal bruta", "V")
        self._adc_readout = ReadoutCard("adc crudo", "/ 4095")
        self._scan_bar = ScanBarCard()

        layout.addWidget(self._lambda_readout, 0, 0)
        layout.addWidget(self._signal_readout, 0, 1)
        layout.addWidget(self._adc_readout, 0, 2)
        layout.addWidget(self._scan_bar, 0, 3)
        layout.setColumnStretch(0, 13)
        layout.setColumnStretch(1, 10)
        layout.setColumnStretch(2, 10)
        layout.setColumnStretch(3, 22)
    ...
    def _update_readouts(self, sample: Sample | None) -> None:
        if sample is None:
            ...
            self._signal_readout.set_value("0.000")
        ...
        self._signal_readout.set_value(f"{sample.voltage_v:.3f}")
