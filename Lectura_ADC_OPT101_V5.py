...
# Calibracion temporal provisional:
# el sensor parte fuera del espectro, cruza el rango util y vuelve a salir
# en aproximadamente 10.66 s. Sin encoder ni contador de pasos, esta es la
# mejor referencia disponible para convertir tiempo de barrido a eje espectral.
SCAN_DURATION_S = 10.66
AUTO_DIRECTION_CHANGE_S = 11.50
SENSOR_SAMPLE_PERIOD_S = 0.300
POINTS_PER_SCAN = max(
    2,
    round((AUTO_DIRECTION_CHANGE_S + SCAN_DURATION_S) / SENSOR_SAMPLE_PERIOD_S) + 4,
)
...
class Sample:
    ...
    @property
    def display_intensity(self) -> float:
        if INTENSITY_MODE.lower() == "raw":
            return float(self.raw)
    ...
    @property
    def display_corrected_intensity(self) -> float:
        responsivity = max(self.responsivity_a_per_w, 1e-9)
        return self.display_intensity / responsivity
...
def wavelength_for_elapsed(elapsed_s: float, direction: int = 1) -> float:
    """Convierte tiempo de barrido a longitud de onda aproximada."""
    ...
    with path.open("w", newline="", encoding="utf-8") as csv_file:
        ...
        writer.writerow(
            [
                ...
                f"auto_direction_change_s={AUTO_DIRECTION_CHANGE_S:.3f}",
            ]
        )
        writer.writerow(
            [
        ...
        for sample in samples:
            writer.writerow(
                [
                    ...
                    f"{sample.display_intensity:.6f}",
                    f"{sample.responsivity_a_per_w:.6f}",
                    f"{sample.corrected_intensity:.6f}",
                    f"{sample.display_corrected_intensity:.6f}",
                ]
            )
...
class SpectrumPlot(pg.PlotWidget):
    ...
    def set_data(self, x_values: list[float], y_values: list[float]) -> None:
        ...
        if y_max > 0:
            self.getPlotItem().setYRange(0, max(y_max * 1.1, 1e-3), padding=0)
...
class Opt101App(QMainWindow):
    ...
    def _build_toolbar(self) -> QWidget:
        ...
        btn_reset.clicked.connect(lambda _checked=False: self._reset())
    ...
    def _read_serial(self) -> None:
        ...
        for _ in range(50):
            ...
            raw, voltage = parsed
            scan_position = self._current_scan_position(now)
            if scan_position is None:
                self._latest_sample = None
                self.statusBar().showMessage(
                    "Muestra recibida sin barrido activo. Usa W, S o I para iniciar t=0."
                )
                continue

            elapsed_s, wavelength = scan_position
            sample = Sample(
    ...
    def _update_plots(self) -> None:
        ...
        raw_display = [s.display_intensity for s in scan]
        corrected_display = [s.display_corrected_intensity for s in scan]
        self.raw_plot.set_data(wls, raw_display)
        self.corrected_plot.set_data(wls, corrected_display)
    ...
    def _reset(self) -> None:
        ...
        self.statusBar().showMessage("Lectura reiniciada. Esperando W, S o I para nuevo t=0.")

    def _begin_timed_scan(self, command: str) -> None:
        self.samples.clear()
        self._latest_sample = None
        self.start_time = time.time()
        self._scan_start_time = self.start_time
        if command == "s":
            self._scan_mode = "down"
        elif command == "i":
    ...
    def _current_scan_position(self, now: float) -> tuple[float, float] | None:
        if self._scan_start_time is None or self._scan_mode == "idle":
            return None

        elapsed_s = now - self._scan_start_time
        if elapsed_s < 0:
            return None

        if self._scan_mode == "up":
            if elapsed_s > SCAN_DURATION_S:
                return None
            return elapsed_s, wavelength_for_elapsed(elapsed_s, direction=1)

        if self._scan_mode == "down":
            if elapsed_s > SCAN_DURATION_S:
                return None
            return elapsed_s, wavelength_for_elapsed(elapsed_s, direction=-1)
        ...
        if self._scan_mode == "auto":
            if elapsed_s <= SCAN_DURATION_S:
                return elapsed_s, wavelength_for_elapsed(elapsed_s, direction=1)
            if elapsed_s < AUTO_DIRECTION_CHANGE_S:
                return None
            return_elapsed = elapsed_s - AUTO_DIRECTION_CHANGE_S
            if return_elapsed > SCAN_DURATION_S:
                return None
            return elapsed_s, wavelength_for_elapsed(return_elapsed, direction=-1)
    ...
    def _send_motor_command(self, command: str) -> None:
        ...
        try:
            ...
            if command in {"w", "s", "i"}:
                self._begin_timed_scan(command)
                self.statusBar().showMessage(
                    f"Comando {command}: t=0 iniciado, barrido temporal {SCAN_DURATION_S:.2f} s"
                )
            elif command == "x":
                self._scan_mode = "idle"
                self._scan_start_time = None
                self.statusBar().showMessage("Comando x: barrido detenido")
            else:
